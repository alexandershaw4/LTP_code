...
% temporal modes. Whereas atcm.integrate weights the states using J, this
% routine weights the modes using J.
%
%    yx   :    DMD(y)                     [DMD applied to region state series]
%    Pf[i]:    L * ( J[i] * fft(wy[i]) )  [weighting over populations, i] 
...
%     % pre svd checks
%     if any(any(isnan(x*x'))) || any(any(isinf(x*x')))
%         [inx,iny] = find(isnan(x*x'));
%         ik = unique([inx(:);iny(:)]);
%         x(ik,:) = 0;
%         [inx,iny] = find(isinf(x*x'));
%         ik = unique([inx(:);iny(:)]);
%         x(ik,:) = 0;
%     end
    [u0,s0,v0] = spm_svd(x);
...
for ins = 1:ns
    ...
    % complexity to a simplistic model
    %----------------------------------------------------------------------
...
%         % Straight fft with post-hoc smoothing
%         %----------------------------------------------------------------
...
%         Pf = smooth(Hz,Pf,0.3,'loess');
        ...
        warning off;
        try
            [Pf,Hz]  = atcm.fun.AfftSmooth(y0(burn:end),1/dt,w,60); % 70
        catch
            Hz = w;
            Pf = w'*0;
        end
